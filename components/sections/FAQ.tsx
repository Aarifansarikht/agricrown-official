...
const FAQ = () => {
  ...
  return (
    <section className="py-24 border-t border-zinc-200 dark:border-zinc-800 transition-colors duration-300">
      <div className="container mx-auto px-4 max-w-3xl">
         <div className="text-center mb-12">
            <span className="text-xs font-bold text-zinc-500 uppercase tracking-widest">FAQ</span>
            <h2 className="text-4xl font-heading font-bold uppercase italic mt-2 text-black dark:text-white">Questions From The Field</h2>
         </div>
         ...
         <div className="space-y-4">
            {faqs.map((faq, idx) => (
              <div key={idx} className="bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 transition-colors duration-300">
                ...
                >
                  <span className={`font-heading font-bold text-lg uppercase italic transition-colors ${openIndex === idx ? 'text-primary' : 'text-black dark:text-white'}`}>
                    {idx + 1 < 10 ? `0${idx + 1}` : idx + 1}. {faq.q}
                  </span>
                  <ChevronDown className={`transform transition-transform ${openIndex === idx ? 'rotate-180 text-primary' : 'text-zinc-500'}`} />
                </button>
                <AnimatePresence>
                  {openIndex === idx && (
                    <motion.div
                    ...
                    >
                      <p className="pb-6 px-6 text-zinc-600 dark:text-zinc-400 text-sm leading-relaxed pl-12 border-l border-zinc-200 dark:border-zinc-700 ml-6">
                        {faq.a}
                      </p>
                    </motion.div>
                  )}
